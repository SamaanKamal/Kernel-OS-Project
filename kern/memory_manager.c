...
{
	//TODO: [PROJECT 2021 - [2] User Heap] malloc() [Kernel Side]

	size = ROUNDUP(size, PAGE_SIZE);
	int pages_no = size/PAGE_SIZE;

	for (int i = 0; i <pages_no ; i++)
		{
			int new_env_page = pf_add_empty_env_page(curenv, virtual_address, 1);

			if(new_env_page == E_NO_PAGE_FILE_SPACE)
			{
...
// [2] freeMem
void delete_empty_pTable (struct Env *e, uint32 virtual_address)
{
	pt_clear_page_table_entry(e,virtual_address);
	uint32 *pTable_ptr=NULL;
	get_page_table (e->env_page_directory,(void*)virtual_address,&pTable_ptr);
	/*get_page_table : is a function Used by “map_frame”
	to get a pointer to the page table if exist
	*/
	int empty_pTable_entry=0;
	for (int j=0; j<1024;j++) //page table entries (1024)
	{
		if( pTable_ptr[j]!=0) //check page table entry
		{
			break;
		}
		empty_pTable_entry++;
	...
	if(empty_pTable_entry==1024)
	{
		uint32* garpage=NULL;
		struct Frame_Info* frameinfo=get_frame_info(e->env_page_directory,(void*)pTable_ptr,&garpage);
		frameinfo->references--;
		if(frameinfo->references==0)
		free_frame(frameinfo);
		pd_clear_page_dir_entry(e,virtual_address);
		//pd_set_table_unused(e,virtual_address);
//		unmap_frame(e->env_page_directory,(void*)pTable_ptr);
//		pd_clear_page_dir_entry(e,virtual_address);
//		pd_set_table_unused(e,virtual_address);
...
{
	//TODO: [PROJECT 2021 - [1] User Heap] free() [Kernel Side]
	...
	{
			if (element->virtual_address >= virtual_address && element->virtual_address < end_va)
			{
				...
				{
					struct WorkingSetElement *secondChanceElement = LIST_FIRST(&(e->SecondList));
					LIST_REMOVE(&(e->SecondList), secondChanceElement);
					LIST_INSERT_TAIL(&(e->ActiveList), secondChanceElement);
					pt_set_page_permissions(e, secondChanceElement->virtual_address, PERM_PRESENT,0);

				}
			//Removes empty page table
				delete_empty_pTable(e,element->virtual_address);

			}
		}
		// Remove element from second chance
