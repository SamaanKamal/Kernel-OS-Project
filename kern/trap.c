...
void page_fault_handler(struct Env * curenv, uint32 fault_va) {
	...
	struct WorkingSetElement * element, *rem_element;

	// sizes initialize
		int Asize = LIST_SIZE(&(curenv->ActiveList));
		int Ssize = LIST_SIZE(&(curenv->SecondList));


	//if fault address exist in second list then add it to active list
	LIST_FOREACH(element,&(curenv->SecondList))
	{
		if (element->virtual_address == fault_va) {
			rem_element = LIST_LAST(&(curenv->ActiveList));
			LIST_REMOVE(&(curenv->ActiveList), rem_element);
			LIST_REMOVE(&(curenv->SecondList), element);
			LIST_INSERT_HEAD(&(curenv->ActiveList), element);
			pt_set_page_permissions(curenv, element->virtual_address,
					PERM_PRESENT| PERM_WRITEABLE | PERM_USER, 0);
			LIST_INSERT_HEAD(&(curenv->SecondList), rem_element);
			pt_set_page_permissions(curenv, rem_element->virtual_address, 0,
					PERM_PRESENT | PERM_WRITEABLE | PERM_USER);
			return;
		}

	}
	//map the frame for fault va in mem
	struct Frame_Info *frame = NULL;
	allocate_frame(&frame);
	map_frame(curenv->env_page_directory, frame, (void *) fault_va,
			PERM_PRESENT | PERM_USER | PERM_WRITEABLE);

	//if page not found in page file check user-stack boundry if true add it to pf
	int flage = pf_read_env_page(curenv, (void *) fault_va);
	if (flage == E_PAGE_NOT_EXIST_IN_PF) {
		if ((fault_va > USTACKBOTTOM && fault_va < USTACKTOP)) {
			pf_add_empty_env_page(curenv, fault_va, 0);
		} else {
			panic("ERROR ADDRESS OUT OF BOUNDDRY !!");
		}
	}


// active list have space
	if (Asize < curenv->ActiveListSize) {
		element = LIST_LAST(&(curenv->PageWorkingSetList));
		element->empty=0;
	...
	// if active list is full and there is no second list
		if(curenv->SecondListSize==0)
		{
			// check the removed element if modified or not
				element = LIST_LAST(&(curenv->ActiveList));

			// return element modified perm
				uint32 Ret_perm = pt_get_page_permissions(curenv,element->virtual_address);
				if (Ret_perm & PERM_MODIFIED) {
					uint32 *ptr_page_table = NULL;

			// make a frame for updated page
				struct Frame_Info *updt_frame = get_frame_info(
				curenv->env_page_directory,(void *) element->virtual_address, &ptr_page_table);

			// update page on disk
				pf_update_env_page(curenv, (void *) element->virtual_address,updt_frame);
				}
				LIST_REMOVE(&(curenv->ActiveList), element);
				pt_set_page_permissions(curenv, element->virtual_address, 0,
				PERM_MODIFIED | PERM_USER | PERM_PRESENT | PERM_WRITEABLE);
		...
		//if the two lists are full
		else if (Ssize == curenv->SecondListSize) {

		// check the removed element if modified or not
			element = LIST_LAST(&(curenv->SecondList));
		...
		//else the second list is not full
